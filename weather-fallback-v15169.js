...
(function(){
  const VERSION='1.5.169';
  const passageTimelineCache=new Map();
  const pointKey=p=>`${Number(p?.lat).toFixed(4)}|${Number(p?.lon).toFixed(4)}|${p?.date||''}|${p?.time||''}`;
  const savePassageTimeline=(p,rows)=>{if(Array.isArray(rows)&&rows.length>=2)passageTimelineCache.set(pointKey(p),rows);};
  const cachedPassageTimeline=p=>passageTimelineCache.get(pointKey(p))||[];
  ...
  function metNoPointBundle(point,payload){
    ...
    savePassageTimeline(point,timeline);
    return {...row,timeline};
  }
  ...
  if(typeof analyzePointsBatch==='function' && typeof providers!=='undefined'){
    analyzePointsBatch=async function(points,providerList=providers,statusLabel='気象モデル'){
      const activeProviders=Array.isArray(providerList)&&providerList.length?providerList:providers;
      const buckets=points.map(()=>({rows:[],errors:[]}));
      for(let pi=0;pi<activeProviders.length;pi++){
        const provider=activeProviders[pi];
        setStatus(`${statusLabel} ${pi+1}/${activeProviders.length}：${provider.name} を全地点まとめて取得中…`);
        try{
      ...
      return points.map((point,index)=>{
        ...
        const avg=(typeof blendProviderRows==='function')?blendProviderRows(rows):averageRows(rows.map(x=>x.row));
        const fallbackOnly=rows.length===1&&rows[0].provider?.kind==='fallback';
        let timelineRows=timelineFromProviderRows(rows);
        if(!Array.isArray(timelineRows)||timelineRows.length<2)timelineRows=cachedPassageTimeline(point);
        else savePassageTimeline(point,timelineRows);
        return {point,providerRows:rows,errors,timelineRows,...avg,grade:assessGrade(avg),confidence:fallbackOnly?'FALLBACK':assessConfidence(rows.map(x=>x.row)),thunder:thunderLevel(avg),hazards:assessHazards(avg)};
      });
    };
  }

  // V1.5.169: progressive model merges can replace result objects.
  // Restore passage timeline immediately before the point card renders.
  if(typeof pointForecastRow==='function'){
    const _pointForecastRow=pointForecastRow;
    pointForecastRow=function(r,...args){
      if(r?.point){
        let tl=Array.isArray(r.timelineRows)?r.timelineRows:[];
        if(tl.length<2)tl=timelineFromProviderRows(r.providerRows||[]);
        if(tl.length<2)tl=cachedPassageTimeline(r.point);
        if(tl.length>=2){r.timelineRows=tl;savePassageTimeline(r.point,tl);}
      }
      return _pointForecastRow(r,...args);
    };
  }
