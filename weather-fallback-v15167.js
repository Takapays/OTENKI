  ...
  if(typeof analyzePointsBatch==='function' && typeof providers!=='undefined'){
    analyzePointsBatch=async function(points){
      ...
      return points.map((point,index)=>{
        ...
        return {point,providerRows:rows,errors,...avg,grade:assessGrade(avg),confidence:fallbackOnly?'FALLBACK':assessConfidence(rows.map(x=>x.row)),thunder:thunderLevel(avg),hazards:assessHazards(avg)};
      });
    };
  }
