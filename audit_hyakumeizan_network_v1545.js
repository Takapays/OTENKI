...
vm.runInContext(s+`\nglobalThis.__X={JAPAN_100_MOUNTAINS,representativeCourseOptions,representativeCourseExpandedPointDefs,directCourseTimeInfoByNames,courseTimeInfo,normalizeCourseTimePointName,canonicalCourseTimeEndpointName,MAJOR_TRAIL_NETWORKS_V1545,GENERATED_ROUTE_EXCLUSIONS_V1545};`,sandbox,{timeout:30000});
const X=sandbox.__X;
...
for(const [area,net] of Object.entries(X.MAJOR_TRAIL_NETWORKS_V1545||{})){
...
const summary={
  ...
  explicitAreaNetworks:Object.keys(X.MAJOR_TRAIL_NETWORKS_V1545||{}).length
};
const payload={summary,scope:'All currently declared Hyakumeizan route options are unioned into a directed network; known composed route edges are expanded to their verified underlying direct CT edges. Kuju adds an explicit branch network beyond representative-route definitions. This audits the source-declared major network and does not claim every real-world minor trail nationwide is cataloged.',exclusions,rows,routeEdges};
fs.writeFileSync('HYAKUMEIZAN_NETWORK_CT_AUDIT_V1545.json',JSON.stringify(payload,null,2));
const lines=[
 'TRATEN V1.5.45 HYAKUMEIZAN CT NETWORK AUDIT','',
...
fs.writeFileSync('HYAKUMEIZAN_NETWORK_CT_AUDIT_V1545.txt',lines.join('\n')+'\n');
