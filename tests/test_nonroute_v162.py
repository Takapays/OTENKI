"""V1.6.2 regression acceptance. All upstream/DB/social traffic is mocked.
...
class RegressionTests(unittest.TestCase):
        ...
        with s.app.test_client() as c:
            for file in ROOT.glob('*.html'):
                ...
                for url in re.findall(r'(?:src|href)=[\"\']([^\"\']+)',file.read_text(errors='replace')):
                    ...
                    r=c.get('/'+path+'?v=1.6.2');self.assertEqual(r.status_code,200,(file.name,path))
    ...
    def test_20_gzip_and_cache_headers(self):
        with s.app.test_client() as c:
            plain=c.get('/app.js?v=1.6.2')
            packed=c.get('/app.js?v=1.6.2',headers={'Accept-Encoding':'gzip'})
    ...
    def test_34_100_prefetch_override_is_effective(self):
    ...
    def test_36_html_local_asset_version_updated_not_external(self):
        with tempfile.TemporaryDirectory() as tmp,patch.object(s,'BASE',tmp):
            ...
            self.assertIn('app.js?v=1.6.2',out);self.assertIn('ui.css?v=1.6.2',out)
    ...
    def test_46_no_immutable_mutable_catalog(self):
        with s.app.test_client() as c:
            r=c.get('/national-100-points.json?v=1.6.2');self.assertNotIn('immutable',r.headers['Cache-Control'])

    def test_47_scheduler_accepts_recoverable_partial_without_hiding_json_failure(self):
        report={'ok':False,'state':'partial','errors':['forecast acquisition incomplete after 429 rate limit'],'rolling':{'dateReports':[{'chunks':[{'rateLimited':True}]}]}}
        with patch.object(s,'NATIONAL_CACHE_REFRESH_TOKEN','tok'),patch.object(s,'_national_supabase_enabled',return_value=True),patch.object(s,'_refresh_national_persistent_cache',return_value=report):
            with s.app.test_client() as c:
                r=c.post('/api/national-outlook/refresh-cache',headers={'X-Traten-Cache-Token':'tok'})
                self.assertEqual(r.status_code,200)
                body=r.get_json();self.assertFalse(body['ok']);self.assertTrue(body['schedulerAccepted'])

    def test_48_scheduler_keeps_database_failure_fatal(self):
        report={'ok':False,'state':'failed','errors':['database write failed for checkpoint'],'rolling':{'dateReports':[]}}
        with patch.object(s,'NATIONAL_CACHE_REFRESH_TOKEN','tok'),patch.object(s,'_national_supabase_enabled',return_value=True),patch.object(s,'_refresh_national_persistent_cache',return_value=report):
            with s.app.test_client() as c:
                r=c.post('/api/national-outlook/refresh-cache',headers={'X-Traten-Cache-Token':'tok'})
