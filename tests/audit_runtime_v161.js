...
for(const name of names){
 ...
 if(!manifest)throw new Error('Frozen baseline required');
 if(digest!==manifest.catalogDigest)throw new Error('Full 300-mountain runtime catalog differs from frozen 230; inspect all differences, never lower the baseline automatically');
 for(const name of names)if(functionHashes[name]!==manifest.functionHashes[name])throw new Error('Protected current JS feature changed: '+name);
 console.log('PASS: whole runtime catalog identical; protected JS functions='+names.length);
...
if(audit.summary.missingCt!==0||audit.summary.coordinateIssues!==0||audit.summary.derivedCt!==0)throw new Error('Catalog safety regression');
