...
function potabilityText(v,x){if(x?.source_name&&x.source_name!=='OpenStreetMap')return '飲用可否は現地確認';return v==='confirmed'?'OSM飲用可登録':v==='not_drinking'?'OSM飲用不可登録':'OSM飲用可否未確認'}
...
function popupHtml(x){
  ...
  const potable=esc(potabilityText(x.potability,x));
  const dist=Number(x.distance_m||0).toLocaleString();
  return `<div class="water-map-popup"><b>💧 ${name}</b><div>${kind}</div><div>${near}から約${dist}m</div><div>${potable}</div></div>`;
}
async function renderMap(m,v){
  ...
  if(!ready){mapEl.innerHTML='<div class="water-map-error">地図ライブラリを読み込めませんでした。水場一覧は下部で確認できます。</div>';return;}
  if(!waterMap){
    mapEl.innerHTML='';
...
function render(){
  ...
  const src=(v.sources||[]).slice(0,20).map(x=>{
    ...
    return `<div class="water-source-mini"><b>💧 ${esc(x.name||x.kind||'水場')}</b><small>${esc(x.kind||'水場')}・${esc(x.near_point||'ルート付近')}から約${Number(x.distance_m||0).toLocaleString()}m・${esc(potabilityText(x.potability,x))}</small>${desc}</div>`;
  }).join('');
