// Traten V1.5.126: remove redundant 3-point representative routes when a verified
// refined route for the same access is already supplied by the existing enrichment bundles.
// No coordinates or CT values are added or changed here.
(function(){'use strict';
const VERSION='1.5.126';
...
if(typeof representativeCourseOptions==='function'){
  ...
  representativeCourseOptions=function(mountain){
    ...
    const k=keyFor(mountain), labels=REDUNDANT[k];
    if(!labels)return opts;
    const remove=new Set(labels);
    return opts.filter(c=>!remove.has(String(c?.label||'')));
  };
}
try{if(typeof rebuildRouteDerivedCaches==='function')rebuildRouteDerivedCaches();}catch(_){}
window.TRATEN_REPRESENTATIVE_ROUTE_CLEANUP_V15126=Object.freeze({
  version:VERSION,
  removedRouteCount:Object.values(REDUNDANT).reduce((n,a)=>n+a.length,0),
  mountains:Object.freeze(Object.keys(REDUNDANT)),
  policy:'remove redundant 3-point options only; keep existing verified refined alternatives; no coordinate/CT changes'
});
})();
