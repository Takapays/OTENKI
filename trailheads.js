...
function populateMountains(){const a=areaSel.value,names=dataMountains.filter(m=>areaMap[m]===a);mountainSel.innerHTML=`<option value="">${a?'山を選択してください':'先に山域を選択してください'}</option>`+names.map(m=>`<option value="${esc(m)}">${esc(m)}</option>`).join('');mountainSel.disabled=!a;render();}
function render(){const m=mountainSel.value;if(!m){summary.innerHTML=`<span>登山口情報のある山 ${dataMountains.length}座</span><span>山域を選んでください</span>`;list.innerHTML='<div class="empty">山域 → 山の順に選択すると、登録済み登山口を表示します。</div>';return;}const names=(byMountain[m]||[]).filter(n=>db[n]);summary.innerHTML=`<span>${esc(m)}</span><span>登山口 ${names.length}地点</span>`;list.innerHTML=names.length?names.map(name=>{const x=db[name];return `<article class="card"><h3>${esc(name)}</h3><div class="meta">${esc(x.area||'')} · ${esc(x.prefecture||'')}</div><p><span class="tag">車</span>${esc(x.nearestIC||'—')} / ${esc(x.drive||'—')}</p><p><span class="tag">駐車</span>${esc(x.parking||'情報なし')}</p><p><span class="tag">公共交通</span>${esc(x.transit||'情報なし')}</p><a class="primary-link" href="trailhead-access.html?name=${encodeURIComponent(name)}&mountain=${encodeURIComponent(m)}">アクセス情報を見る →</a></article>`}).join(''):'<div class="empty">この山のアクセス登録済み登山口はありません。</div>';}
