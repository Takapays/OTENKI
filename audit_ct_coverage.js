...
const OUT_TXT=path.join(ROOT,'CT_COVERAGE_AUDIT_V14127.txt');
const OUT_CSV=path.join(ROOT,'CT_MISSING_SEGMENTS_V14127.csv');
const OUT_EST=path.join(ROOT,'CT_ESTIMATED_SEGMENTS_V14127.csv');
...
const lines=[
  ...
  '【解釈】',
  '- 確認済みCTは常に推定CTより優先。',
  '- V1.4.127では未確認の固定登山口↔山頂にのみ推定CTを適用し、画面では「推定CT」と明示する。',
